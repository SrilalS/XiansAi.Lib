...
| Combined size (decoded) | 20 MB |
| Combined size of new files, from workflow code | 1,500,000 base64 chars (~1.1 MB of bytes) |
| File name length | 255 chars |

The workflow-code limit is measured on the base64 text, because that is what Temporal stores in workflow history. Exceeding it fails the workflow with a message quoting the character counts, not megabytes.
...
public class ReportWorkflow
{
    ...
    {
        // Generated bytes pass through workflow history — keep them near 1 MB or less.
